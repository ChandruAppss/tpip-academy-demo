...
export default function VideoCallModal({ isOpen, onClose, sessionId }) {
  const {
    ...
    initCamera, endCall, toggleMic, toggleCamera,
  ...
  useEffect(() => {
    if (!isOpen) return
    initCamera().then(ok => {
      if (ok) toast.success('Camera and mic ready — waiting for student to call')
    })
    if (sessionId) {
      localStorage.setItem(`tutor_request_${sessionId}`, JSON.stringify({ timestamp: new Date().toISOString(), coach: true }))
    }
  }, [isOpen])

  // Sync chat via localStorage polling (bidirectional)
  useEffect(() => {
  ...
  const handleSend = () => {
    ...
    const msg = { id: Date.now(), sender: 'coach', text: msgInput.trim(), timestamp: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) }
  ...
  const copyId = () => {
    ...
    toast.success('Peer ID copied! Share with your student.')
  }
      ...
      <div style={{ width:'95%', maxWidth:1200, background:BG, border:`1px solid ${BORDER}`, borderRadius:16, overflow:'hidden', boxShadow:'0 20px 60px rgba(0,0,0,0.7)', display:'flex', flexDirection:'column', maxHeight:'92vh' }}
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'18px 24px', borderBottom:`1px solid ${BORDER}` }}>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            <div>
              <div style={{ fontSize:18, fontWeight:700, color:'#fff' }}>Live Coaching Session</div>
              <div style={{ fontSize:12, color: inCall ? LIME : MUTED }}>
                {inCall ? 'Student connected — session in progress' : cameraReady ? 'Ready — share your peer ID with the student' : 'Initialising camera…'}
              </div>
            </div>
          </div>
          ...
          <div style={{ flex:1, display:'flex', flexDirection:'column', gap:12, minWidth:0 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, flex:1, minHeight:300 }}>
              ...
              <div style={{ background:'#000', borderRadius:12, overflow:'hidden', position:'relative', display:'flex', alignItems:'center', justifyContent:'center', border:`1px solid ${BORDER}` }}>
                ...
                {!inCall && (
                  <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.75)', color:MUTED, textAlign:'center', gap:10 }}>
                    <div style={{ fontSize:38 }}>📱</div>
                    <div style={{ fontSize:14, fontWeight:600, color:'#fff' }}>Waiting for student…</div>
                    <div style={{ fontSize:12 }}>Share your peer ID below — student will call you</div>
                  </div>
                )}
                <div style={{ position:'absolute', bottom:10, left:12, fontSize:11, background:'rgba(0,0,0,0.65)', padding:'3px 8px', borderRadius:5, color:MUTED }}>Student</div>
              </div>
              ...
              <div style={{ background:'#000', borderRadius:12, overflow:'hidden', position:'relative', display:'flex', alignItems:'center', justifyContent:'center', border:`1px solid ${BORDER}` }}>
                ...
                <div style={{ position:'absolute', bottom:10, left:12, fontSize:11, background:'rgba(0,0,0,0.65)', padding:'3px 8px', borderRadius:5, color:LIME }}>You (Coach)</div>
                {isCamOff && (
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, justifyContent:'center', flexWrap:'wrap' }}>
              ...
              {inCall && (
                <button onClick={handleClose} style={{ padding:'9px 24px', background:RED, border:'none', borderRadius:10, color:'#fff', fontWeight:700, fontSize:13, cursor:'pointer' }}>
                  📵 End Session
                </button>
              )}
            </div>
            ...
            <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:12, padding:16 }}>
              <label style={{ display:'block', fontSize:11, fontWeight:700, color:MUTED, marginBottom:8, textTransform:'uppercase', letterSpacing:'0.8px' }}>
                Your Peer ID — Share this with your student
              </label>
              <div style={{ display:'flex', gap:8 }}>
                <input value={peerId || 'Loading…'} disabled style={{ flex:1, padding:'10px 14px', background:'rgba(0,0,0,0.4)', border:`1px solid ${BORDER}`, borderRadius:8, color:'#fff', fontSize:13, fontFamily:'monospace', fontWeight:500 }}/>
                <button onClick={copyId} style={{ padding:'10px 20px', background:PURPLE, border:'none', borderRadius:8, color:'#fff', fontWeight:600, cursor:'pointer', fontSize:13, whiteSpace:'nowrap' }}>
                  {copied ? '✓ Copied' : 'Copy ID'}
                </button>
              </div>
              <div style={{ fontSize:11, color:MUTED, marginTop:8 }}>Student pastes this ID in their session screen to call you</div>
            </div>
          </div>

          {/* Chat sidebar — bidirectional */}
          <div style={{ width:280, background:CARD, border:`1px solid ${BORDER}`, borderRadius:12, display:'flex', flexDirection:'column', overflow:'hidden', flexShrink:0 }}>
            ...
            <div style={{ flex:1, overflowY:'auto', padding:12, display:'flex', flexDirection:'column', gap:8 }}>
              {messages.length === 0
                ...
                : messages.map(msg => (
                  <div key={msg.id} style={{ background: msg.sender === 'coach' ? `${LIME}18` : `${PURPLE}33`, border:`1px solid ${msg.sender === 'coach' ? LIME+'40' : PURPLE+'55'}`, borderRadius:8, padding:'8px 10px', fontSize:12, color:'#fff', wordBreak:'break-word' }}>
                    <div style={{ fontSize:10, color:MUTED, marginBottom:3 }}>{msg.sender === 'coach' ? 'You (Coach)' : 'Student'} · {msg.timestamp}</div>
                    {msg.text}
                  </div>
                ))
              }
            ...
            <div style={{ padding:'10px 10px', borderTop:`1px solid ${BORDER}`, display:'flex', gap:8 }}>
              <input value={msgInput} onChange={e => setMsgInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder="Reply to student…"
