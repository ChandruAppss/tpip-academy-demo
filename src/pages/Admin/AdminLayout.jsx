...
export default function AdminLayout() {
  ...
  const initials = getInitials(profile?.full_name || 'Admin User')

  function handleLogout() { logout(); navigate('/login') }

  const LEFT_NAV = [
    { label: 'Dashboard',    to: '/admin',              end: true, color: LIME },
    { label: 'Students',     to: '/admin/students',                color: '#a855f7' },
    { label: 'Coaches',      to: '/admin/coaches',                 color: '#22d3ee' },
    { label: 'Programs',     to: '/admin/programs',                color: '#3b82f6' },
    { label: 'Sessions',     to: '/admin/sessions',                color: '#f97316' },
    { label: 'Certificates', to: '/admin/certificates',            color: '#eab308' },
  ]
  const RIGHT_NAV = [
    { label: 'Academies',    to: '/admin/academies',               color: '#ef4444' },
    { label: 'Payments',     to: '/admin/payments',                color: '#06b6d4' },
    { label: 'Payouts',      to: '/admin/payouts',                 color: '#10b981' },
    { label: 'AI Settings',  to: '/admin/ai-settings',             color: '#8d59ff' },
    { label: 'Settings',     to: '/admin/settings',                color: '#9ca3af' },
  ]
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', fontFamily:'system-ui,-apple-system,sans-serif', background:BG, color:'#fff' }}>
      ...
      <div style={{ position:'relative', background:CARD, borderBottom:`1px solid ${BORDER}`, display:'flex', alignItems:'center', padding:'0 16px', height:80, flexShrink:0 }}>
        ...
        <div style={{ position:'absolute', left:'50%', transform:'translateX(-50%)', display:'flex', flexDirection:'column', alignItems:'center' }}>
          ...
          />
          <div style={{ fontSize:9, color:'rgba(255,255,255,0.4)', letterSpacing:'1.5px', textTransform:'uppercase', marginTop:2 }}>ADMIN PANEL</div>
        </div>
